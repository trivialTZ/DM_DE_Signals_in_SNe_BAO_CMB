...
def sne_distmod(z, H0, Om0, w0, wa, cosmo=None):
    if cosmo is None:
        model = Flatw0waCDM(H0=H0, Om0=Om0, w0=w0, wa=wa)
    else:
        model = cosmo
...
def log_likelihood(params, z, data_obs, data_err, type, H0 = 70.0):
    ...
    if type == 'sne_luminosity_distance':
        data_model = sne_luminosity_distance(z, H0=70.0, Om0=Om0, w0=w0, wa=wa)
        chi2 = np.sum(((data_obs - data_model) / data_err) ** 2)
        ...
        data_model = sne_distmod(z, H0=70.0, Om0=Om0, w0=w0, wa=wa)
        chi2 = np.sum(((data_obs - data_model) / data_err) ** 2)
    elif type == 'sne_distmod_with_H0':
        data_model = sne_distmod(z, H0=H0, Om0=Om0, w0=w0, wa=wa)
        chi2 = np.sum(((data_obs - data_model) / data_err) ** 2)
    elif type == 'dv':
        ...
        data_model = sne_distmod(z, H0=70.0, Om0=Om0, w0=-1, wa=0)
        chi2 = np.sum(((data_obs - data_model) / data_err) ** 2)
    elif type == 'sne_lcdm_distmod_with_H0':
        data_model = sne_distmod(z, H0=H0, Om0=Om0, w0=-1, wa=0)
        chi2 = np.sum(((data_obs - data_model) / data_err) ** 2)
        ...
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_luminosity_distance(z_sne, H0=70.0, Om0=Om0, w0=w0, wa=wa)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=70.0, Om0=Om0, w0=w0, wa=wa)
            chi2_dv = np.sum(((data_obs['dv'] - data_model_dv) / data_err['dv']) ** 2)
        if 'fap' in data_obs:
            z_fap = z['fap']
            data_model_fap = bao_fap(z_fap, H0=70.0, Om0=Om0, w0=w0, wa=wa)
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
        ...
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_luminosity_distance(z_sne, H0=H0, Om0=Om0, w0=w0, wa=wa)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=H0, Om0=Om0, w0=w0, wa=wa)
            chi2_dv = np.sum(((data_obs['dv'] - data_model_dv) / data_err['dv']) ** 2)
        if 'fap' in data_obs:
            z_fap = z['fap']
            data_model_fap = bao_fap(z_fap, H0=H0, Om0=Om0, w0=w0, wa=wa)
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
    elif type == 'combined_luminosity_distance_lcdm':
        chi2_sne = 0
        chi2_dv = 0
        chi2_fap = 0
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_luminosity_distance(z_sne, H0=70.0, Om0=Om0, w0=-1, wa=0)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=70.0, Om0=Om0, w0=-1, wa=0)
            chi2_dv = np.sum(((data_obs['dv'] - data_model_dv) / data_err['dv']) ** 2)
        if 'fap' in data_obs:
            z_fap = z['fap']
            data_model_fap = bao_fap(z_fap, H0=70.0, Om0=Om0, w0=-1, wa=0)
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
    elif type == 'combined_luminosity_distance_lcdm_with_H0':
        chi2_sne = 0
        chi2_dv = 0
        chi2_fap = 0
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_luminosity_distance(z_sne, H0=H0, Om0=Om0, w0=-1, wa=0)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=H0, Om0=Om0, w0=-1, wa=0)
            chi2_dv = np.sum(((data_obs['dv'] - data_model_dv) / data_err['dv']) ** 2)
        if 'fap' in data_obs:
            z_fap = z['fap']
            data_model_fap = bao_fap(z_fap, H0=H0, Om0=Om0, w0=-1, wa=0)
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
        ...
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_distmod(z_sne, H0=70.0, Om0=Om0, w0=w0, wa=wa)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=70.0, Om0=Om0, w0=w0, wa=wa)
        ...
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_distmod(z_sne, H0=H0, Om0=Om0, w0=w0, wa=wa)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=H0, Om0=Om0, w0=w0, wa=wa)
        ...
        if 'fap' in data_obs:
            ...
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
    elif type == 'combined_distmod_lcdm':
        chi2_sne = 0
        chi2_dv = 0
        chi2_fap = 0
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_distmod(z_sne, H0=70.0, Om0=Om0, w0=-1, wa=0)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=70.0, Om0=Om0, w0=-1, wa=0)
        ...
        if 'fap' in data_obs:
            ...
            chi2_fap = np.sum(((data_obs['fap'] - data_model_fap) / data_err['fap']) ** 2)
        chi2 = chi2_sne + chi2_dv + chi2_fap
    elif type == 'combined_distmod_lcdm_with_H0':
        chi2_sne = 0
        chi2_dv = 0
        chi2_fap = 0
        if 'sne' in data_obs:
            z_sne = z['sne']
            data_mode_snel = sne_distmod(z_sne, H0=H0, Om0=Om0, w0=-1, wa=0)
            chi2_sne = np.sum(((data_obs['sne'] - data_mode_snel) / data_err['sne']) ** 2)
        if 'dv' in data_obs:
            z_dv = z['dv']
            data_model_dv = bao_dv_dr(z_dv, H0=H0, Om0=Om0, w0=-1, wa=0)
...
def run_mcmc(z, data_obs, data_err, type, initial_guess, ndim=3, nwalkers=50, nsteps=5000, nburn=100, nthin=15):
...
def model_to_lcdm_cost_function(omm, z, data_obs, data_err, type='sne_lcdm_distmod', cov_matrix_sne=None, H0=70.0):
    _type = type_to_lcdm(type)

    return -log_likelihood(omm, z, data_obs, data_err, type=_type , H0=H0)


def model_to_lcdm_cost_function_with_H0(params, z, data_obs, data_err, type='sne_lcdm_distmod_with_H0', cov_matrix_sne=None):
    _type = type_to_lcdm(type)
    return -log_likelihood(params, z, data_obs, data_err, type=_type)


def data_obs_from_waw0_cosmology(z, w0, wa, om0, type_for_cosmo, H0=70.0):
    ...
    if _type == 'sne_luminosity_distance_lcdm' or _type == 'sne_luminosity_distance_lcdm_with_H0':
        ...
        data_obs = sne_distmod(z=z, H0=H0, Om0=om0, w0=w0, wa=wa, cosmo=w0wacosmology)
        ...
        if 'sne' in z:
            z_sne = z['sne']
            data_obs['sne'] = sne_distmod(z_sne, H0=H0, Om0=om0, w0=w0, wa=wa, cosmo=w0wacosmology)
        if 'dv' in z:
            z_dv = z['dv']
            data_obs['dv'] = bao_dv_dr(z_dv, H0=H0, Om0=om0, w0=w0, wa=wa)
        if 'fap' in z:
...
def worker_run_gussian_om0_with_H0(args):
    ...
    result_sne = minimize(model_to_lcdm_cost_function_with_H0, np.array([omm_initial, h0_initial]),
                          args=(snez, data_obs_sne, errsne, 'sne_distmod_with_H0'),
                          method='BFGS')
    ...
    hessian_sne_at_map = hessian_neg_log_posterior(theta_map_sne, model_to_lcdm_cost_function_with_H0,
                                                   (snez, data_obs_sne, errsne, 'sne_distmod_with_H0'))
...
def run_gaussian_methods_om0_with_H0_by_multiprocessing(param_combinations, z, data_err):
    args = [(params, z, data_err) for params in param_combinations]
    results = []
    with get_context("spawn").Pool() as pool:
...
def run_lcdm_methods_om0_by_multiprocessing(param_combinations, z, data_err):
    args = [(params, z, data_err) for params in param_combinations]
    results = []
    with get_context("spawn").Pool() as pool:
...
def run_lcdm_methods_om0_by_multiprocessing_with_H0(param_combinations, z, data_err):
    args = [(params, z, data_err) for params in param_combinations]
    results = []
    with get_context("spawn").Pool() as pool:
