...
_cov_matrix_sne = np.diag(MUERR_FINAL_array ** 2) + C_SYS
...
def log_likelihood(params, z, data_obs, data_err, type, H0 = 70.0, cov_matrix_sne=None):
    ...
    if cov_matrix_sne is None:
        cov_matrix_sne=  _cov_matrix_sne
    ...
    if type == 'sne_luminosity_distance':
        ...
        if 'sne' in data_obs:
            z_sne = z['sne']
            if 'distmod' in type:
                data_model_sne = sne_distmod(z_sne, H0=H0 if 'with_H0' in type else 70.0, Om0=Om0, w0=w0, wa=wa)
            else:
...
def run_mcmc_with_cov(z, data_obs, data_err, type, initial_guess, ndim=3, nwalkers=50, nsteps=5000, nburn=100, nthin=15, cov_matrix_sne= None):
    pos = [initial_guess + 1e-4 * np.random.randn(ndim) for i in range(nwalkers)]

    with Pool() as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_likelihood, args=(z, data_obs, data_err, type,cov_matrix_sne), pool=pool)
...
def model_to_lcdm_cost_function(omm, z, data_obs, data_err, type='sne_lcdm_distmod', cov_matrix_sne=None, H0=70.0):
    ...
    return -log_likelihood(omm, z, data_obs, data_err, type=_type,  cov_matrix_sne=cov_matrix_sne, H0=H0)


def model_to_lcdm_cost_function_with_H0(params, z, data_obs, data_err, type='sne_lcdm_distmod_with_H0',  cov_matrix_sne=None):
    _type = type_to_lcdm(type)
    return -log_likelihood(params, z, data_obs, data_err, type=_type,  cov_matrix_sne=cov_matrix_sne)
